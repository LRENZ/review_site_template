...
export default function Collection() {
  ...
  return (
    <Layout>
      <SEO 
        title={`${collection.title} | DailyChoice Collections`}
      ...
      <div className="relative h-[80vh] min-h-[600px] w-full overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-16 lg:p-24">
          <div className="container mx-auto max-w-6xl">
            <Link href="/">
              <a className="inline-flex items-center text-sm font-medium text-white/80 hover:text-white mb-8 transition-colors uppercase tracking-widest border border-white/20 px-4 py-2 rounded-full backdrop-blur-sm bg-black/10">
                <ArrowLeft className="w-4 h-4 mr-2" /> DailyChoice Collections
              </a>
            </Link>
            ...
            <div key={item.reviewSlug} className={cn(
              ...
              <div className="w-full lg:w-1/2">
                <Link href={`/review/${review.slug}`}>
                  <a className="block overflow-hidden rounded-sm relative aspect-[4/5] shadow-2xl cursor-pointer">
                    <img 
              ...
              <div className="w-full lg:w-1/2 space-y-8">
                <div className="space-y-4">
                  ...
                  <h2 className="text-4xl md:text-5xl font-serif font-bold leading-tight">
                    <Link href={`/review/${review.slug}`}>
                      <a className="hover:text-primary transition-colors hover:underline decoration-2 underline-offset-4">
                        {review.title}
                      </a>
                    </Link>
                  </h2>
      ...
      {/* Footer Navigation */}
      <div className="bg-secondary/30 py-24 mt-24 border-t">
        <div className="container mx-auto px-4 text-center">
          <h3 className="text-2xl font-serif font-bold mb-8">More Collections</h3>
          <div className="flex flex-wrap justify-center gap-4">
            {collections.filter(c => c.slug !== slug).map(c => (
              <Button key={c.id} variant="outline" size="lg" className="h-14 px-8 text-lg font-serif" asChild>
